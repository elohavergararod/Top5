  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const data: ListFormData = {
        title: title.trim(),
        category,
        items: items.map(item => ({ name: item.name.trim(), description: item.description.trim() })),
      }
      const created = await addList(data)
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1.5">List title</label>
          <input
            type="text"
            value={title}
            onChange={e => { setTitle(e.target.value); setErrors(p => ({ ...p, title: '' })) }}
            placeholder="e.g. Best films of all time"
            className={`w-full border rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-stone-300 transition ${errors.title ? 'border-red-300' : 'border-stone-200'}`}
          />
        ...
        <div>
          <p className="block text-sm font-medium text-stone-700 mb-3">Your Top 5 — in order</p>
              ...
              <div key={i} className="flex gap-3 items-start">
                ...
                <div className="flex-1 space-y-1.5">
                  <input
                    ...
                    className={`w-full border rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-stone-300 transition ${errors[`item-${i}`] ? 'border-red-300' : 'border-stone-200'}`}
                  />
                  {errors[`item-${i}`] && <p className="text-xs text-red-500">{errors[`item-${i}`]}</p>}
                  <input
                    type="text"
        ...
        >
          {submitting ? 'Saving…' : 'Publish list'}
        </button>
      </form>
    </div>
  )
