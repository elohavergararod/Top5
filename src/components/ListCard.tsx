...
export default function ListCard({ list }: Props) {
  return (
    ...
    >
      ...
      <h2 className="font-semibold text-stone-900 dark:text-stone-100 text-base mb-3 leading-snug group-hover:text-stone-600 dark:group-hover:text-stone-300 transition-colors">
        {list.title}
      </h2>
      ...
      <ol className="space-y-1.5">
        {list.items.map(item => (
          <li key={item.id} className="flex items-start gap-2.5 text-sm">
            <span className="text-xs font-bold text-stone-300 dark:text-stone-600 mt-0.5 w-4 shrink-0">
              {item.rank}
            </span>
